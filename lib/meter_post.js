...
async function postMeterData(cookies, m) {
    ...
    try {
        ...
        if (!pbs) return { success: false, reason: "Session Expired" };
        ...
        constYZtriggerRes = await client.post(url, triggerPayload, {
            headers: { 'Cookie': cookieStr, 'Content-Type': 'application/x-www-form-urlencoded', 'X-MicrosoftAjax': 'Delta=true' }
        });
        ...
        return { success: isSuccess, reason: isSuccess ? "Saved Successfully" : "Failed", isDuplicate: finalRes.data.includes('Already Exists') };
    } catch (e) { return { success: false, reason: "Proxy Error" }; }
}
