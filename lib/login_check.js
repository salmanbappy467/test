...
async function verifyLoginDetails(userid, password) {
    try {
        ...
        if (!cookies || cookies.length === 0) return { success: false, message: "Login Failed (Check User/Pass)" };
        ...
        if (userInfo.includes(',')) zonalName = userInfo.split(',').pop().replace(']', '').trim();

        return { success: true, cookies: cookies, userInfo: userInfo, pbs: pbsName || "N/A", zonal: zonalName };
    } catch (error) {
        return { success: false, message: "Proxy Timeout or Error" };
    }
}
